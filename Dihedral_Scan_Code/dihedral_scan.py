...
class Dh_Info: #This class builds all the information needed to create the dihedral scan either by scrapping or user prompting
    def __init__(self, backbone_indexes=None, scan_index=None, charges=None , DFT_Energy=None, MD_Energy=None, Parameters=None):
        ...
        #not working rn for some reason, should make sure the order of dihedrals is consitent
        ...
        if temp_dh == []:
            scan_test = list(reversed(scan_index))
            
            temp_dh = Scan.check_dihedral(self,dihedral_atoms=scan_test,base_itp="base.itp")
            
            if temp_dh != []:
                scan_index = temp_dh[0] #not sure why this is need but it keeps the output clean :---/
                print(f'Changing Scan Index to match .itp: {scan_index}')
...
################################
    ...
    def read_charges_from_log(self):
        ...
        for line_number, line in enumerate(lines, start=1):
            ...
            if match_start:
                charges = []  # Clear charges list when a new pattern start is found
                Read = True
                continue 
...
class Scan:
    ...
    def update_charge_column(self,input_file, charges):
        ...
        for idx, charge in zip(range(start_idx + 2, end_idx), charges):
            ...
            line_parts[-2] = f"{float(charge):.6f}"  # Replace the charge value with 6 decimal places
    ...
    def check_dihedral(self,dihedral_atoms, base_itp=None):
        ...
        dihedral_atoms = [int(atom) for atom in dihedral_atoms]
    ...
    def find_dihedral_line_and_replace(self,itp_file, dihedral_atoms, new_parameters):
        ...
        if not found_line:
            print(f"Error: No line with specified dihedral atoms({dihedral_atoms}) found in {itp_file}.")
            return 
