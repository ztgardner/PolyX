...
class Itp_parser:
    ...
    def write_gro(
        ...
        *,
        title:            str          = "Generated by write_gro",
        resname:          str          = "RES",
        atomname:         str          = "ATOM",           # default for all
        atomnames:        list[str]  | None = None,       # Use ITP for self.DF["atoms"]["atom_name"]
        residue_number:   int          = 1,
        start_index:      int          = 1
    ) -> None:
        """
        Write a GROMACS .gro file, allowing per-atom names.
        ...
        coords : list of (x,y,z)
            Coordinates in nm.
        box : list of float or None
            [Lx,Ly,Lz] or 9-element box, or None → zeros.
        title : str
            Title line.
        resname : str
            5-char residue name.
        atomname : str
            Fallback atom name (5 chars).
        atomnames : list of str, optional
            If given, must be len(coords); per-atom 5-char names.
        residue_number : int
            Residue index for all atoms.
        start_index : int
            First atom index.
        """
        n_atoms = len(coords)
        if atomnames is not None and len(atomnames) != n_atoms:
            raise ValueError("`atomnames` must have the same length as `coords`")

        # Prepare box line
        if box is None:
            box_line = f"{0.000:10.5f}{0.000:10.5f}{0.000:10.5f}"
        else:
            if len(box) == 3:
                box_line = f"{box[0]:10.5f}{box[1]:10.5f}{box[2]:10.5f}"
            elif len(box) == 9:
                box_line = "".join(f"{v:10.5f}" for v in box)
            else:
                raise ValueError("box must be length 3 or 9")

        with open(filename, "w") as gro:
            gro.write(f"{title}\n")
            gro.write(f"{n_atoms:5d}\n")

            idx = start_index
            for i, (x, y, z) in enumerate(coords):
                name = atomnames[i] if atomnames is not None else atomname
                # ensure name is max 5 chars
                name = name[:5].rjust(5)
