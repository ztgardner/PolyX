...
dropdown.addEventListener("change", () => {
    populateTextbox(); // Update textbox when dropdown changes
...
//Populate text, if not string input uses dropdown selection
function populateTextbox(custom_string) {
    const textBox = document.getElementById("custom-textbox");

    if (!custom_string) {
        if (!itpSection.includes(dropdown.value)) {
            textBox.value = "No ITP Section Info Found";
            return;
        }

        const section = jsonObject[dropdown.value];
        let out_lines = [];

        // Top comment
        if (section.Comments_top) {
            out_lines.push(section.Comments_top);
        }

        // Determine which keys to include (all except comments)
        const dataKeys = Object.keys(section)
            .filter(k => k !== "Comments_top" && k !== "Comments_bottom");

        // Find how many rows we need (the longest array among those keys)
        const numRows = dataKeys.reduce((max, k) => {
            const arr = section[k];
            return Array.isArray(arr) ? Math.max(max, arr.length) : max;
        }, 0);

        // Build each row by zipping across all keys
        for (let i = 0; i < numRows; i++) {
            const chunks = dataKeys.map(key => {
                const arr = section[key];
                if (!Array.isArray(arr) || !arr[i]) {
                    return "";
                }
                // Turn [a,b,c] into "a,b,c", or scalar into "x"
                const entry = arr[i];
                return Array.isArray(entry) ? entry.join(" ") : String(entry);
            });
            // join key-chunks with a space between them
            out_lines.push(chunks.join("     "));
        }

        // Bottom comment
        if (section.Comments_bottom) {
            out_lines.push(section.Comments_bottom);
        }

        textBox.value = out_lines.join("\n");
    } else {
        textBox.value = custom_string;
    }
}
    ...
    Plotly.newPlot('plot-container', dataForPlot, layout).then(() => {
        ...
        plotContainer.on('plotly_click', (data) => {
            ...
            if (data && data.points && data.points.length > 0) {
                ...
                console.log(`Clicked point: (${pointData.x}, ${pointData.y}, ${pointData.z}), Plotly Index: ${pointData.pointNumber}, ITP Index: ${pointData.pointNumber + 1}`);

                //making sure the clicked point is an actual index
                if (pointData.pointNumber >= 0 && pointData.pointNumber <= x.length) {

                    plotContainer.removeAllListeners('plotly_click'); //removes listeners to stop it looping
                    onAtomClick(pointData) //logic for when an atom is successfully clicked
                    plotContainer.on('plotly_click', arguments.callee); //reattach listeners

                } else {
                    console.warn('Clicked Point out of Scope')
                }
...
function onAtomClick(atomInfo){
    //Gets called when an atom is sucessfully cliked, changes the atoms color, updats vectors, draws vecotrs

    //Change Atom Color
    const index = atomInfo.pointNumber

    const new_color = [...color]
    new_color[index] = 'red'
    const size = new Array(new_color.length).fill(15) //Creates an array to set the sizes of the atoms
    size[index] = 15




    //Find Atom Connections
    atomInfoFromIndex(index)
    ...
    Plotly.update('plot-container', {
        'marker.color': [new_color], // Highlight color
        //'marker.size': [size],     // Highlight size
    }, [index]);
}
...
function atomInfoFromIndex(index){
    ...
    //const relaventParams = jsonObject[section]
    //console.log(relaventParams)
    let string  = atoms.filter(list => list.includes(String(index +1)));//Add one to get ptoly index to itp
