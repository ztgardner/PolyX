...
#############################################################
output_path = 'output.gro'          # What would you like the output file to be named?
file_path = 'monomer.gro'           # What is the name of the input file?
dihedral_file_path = 'test.txt'     # What is the name of the file listing all the dihedrals?
start_atoms = [27,11,10]            # The list of atoms that start the polymer, going into the polymer H-C-S for thiophene
end_atoms = [10, 9,26]              # The list of atoms that end the polymer, coming out of the polymer S-C-H for thiophene
n = 76                              # How long do you want the polymer?
bond_length = 0.15
#############################################################
...
def fix_propagation_points(df, start_atoms, end_atoms):
    # Ensure atom_num is treated as an integer
    df = df.copy()

    # Create a mapping of atom_num to index
    atom_index = {num: idx for idx, num in enumerate(df["atom_num"])}

    # Sort based on start_atom[0] as the first, end_atom[-1] as the last
    sorted_indices = sorted(df.index, key=lambda i: (
        i != atom_index[start_atoms[0]],  # Force start_atom[0] to be first
        i == atom_index[end_atoms[-1]],   # Force end_atom[-1] to be last
        i                                  # Preserve order otherwise
    ))
...
def translate_points_to_distance(df, target_distance):
...
def rotate_points_dihedral(df, target_dihedral_angle):
...
def rotate_points_alt_dihedral(df, target_dihedral_angle):
    target_dihedral_angle = target_dihedral_angle * (-1)
...
def run_operations(df, n, dihedral_angles):
    ...
    for i in range(n-1):
        ...
        df = rotate_points(df)
        df = rotate_points_triangle(df)
        df = translate_points_to_distance(df, bond_length)
        df = rotate_points_dihedral(df, dihedral_angle)  # Use the selected angle
...
def print_coordinates(df):
    # Extract coordinates for nmer=1 and nmer=2
    nmer_1_coords = df[['x', 'y', 'z']].values.tolist()

    # Print coordinates in the specified format
    print('\n'.join([f'({", ".join(map(str, coord))})' for coord in nmer_1_coords]))
...
df1 = parse_gro(file_path)
df2 = fix_propagation_points(df1, start_atoms, end_atoms)
df3 = split_atom_name(df2)
df4 = monomer(df3, start_atoms, end_atoms)
df5 = prep_nmer(df4, n)

# Run the polymerization process using the dihedral angles from the file
df6 = run_operations(df5, n, dihedral_angles)

# Final update after the loop
df7 = update_dataframe_with_hex_style(df6)

# Process and write the final DataFrame to a GRO file
processed_df = process_dataframe(df7)
